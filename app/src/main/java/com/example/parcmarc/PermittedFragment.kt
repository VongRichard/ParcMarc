...
open class PermittedFragment : Fragment() {
    ...
    fun requestPermissions(permissions: Array<String>, requestId: Int, onSuccess: () -> Unit, onFailure: () -> Unit) {
        ...
        val needsWriteSettings = hasWriteSettings && !Settings.System.canWrite(context)
        ...
        val ungranted = when {
            ...
            else -> remaining.filter { context?.let { it1 -> ActivityCompat.checkSelfPermission(it1, it) } != PackageManager.PERMISSION_GRANTED }
        }
